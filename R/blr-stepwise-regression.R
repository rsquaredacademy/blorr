...
blr_step_p_both.default <- function(model, pent = 0.1, prem = 0.3, details = FALSE, ...) {
  ...
  df       <- nrow(l) - 2
  tenter   <- stats::qt(1 - (pent) / 2, df)
  trem     <- stats::qt(1 - (prem) / 2, df)
  n        <- ncol(l)
  all_pred <- nam
  cterms   <- all_pred
  ...
  while (step < mlen_p) {
    ...
    if (pvals[minp] <= pent) {
      ...
      m2     <- stats::glm(paste(response, "~", paste(preds, collapse = " + ")), l, 
                  family = stats::binomial(link = 'logit')) 
      m_sum <- car::Anova(m2, test.statistic = "Wald")
      ...
      cat("\n")
      cat(crayon::bold$red(glue::glue("No more variables to be added/removed.")))
      cat("\n")
      break
    }
  }
